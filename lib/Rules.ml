...
let program () =
  ...
  (* print_endline "Case 1";
     ...
     environment
     |> Env.add_constructor "Just"
          (Regular
             (Forall
                ( [ "a" ],
                  [],
                  Function
                    (Skolem "a", Application (Constructor "Maybe", Skolem "a")) )));

     let t, c =
       Infer.infer environment
         (Case
            ( Apply (Constructor "Just", Int 42),
              PtApply (PtConstructor "Just", [ PtVariable "a" ]),
              Variable "a" ))
     in
     print_endline @@ Pretty.render_ty @@ Type.normalize t;
     c
     |> List.iter (fun (Predicate p) ->
            print_endline @@ Pretty.render_ty_predicate @@ Predicate.normalize p); *)
  environment
  |> Env.add_constructor "HasEq"
       (Constructor
          ( [ "a" ],
            Forall
              ( [],
                [ Eq (Skolem "a") ],
                Function (Skolem "a", Constructor "HasEq") ) ));
  ...
  let c = Solver.solve environment c in
  print_endline @@ Pretty.render_ty @@ Type.normalize t;
  c |> List.iter (fun c -> print_endline @@ Pretty.render_ty_constraint c)

(* environment
   |> Env.add_constructor "HasUnify"
        (Generalized
           ( [],
             Forall
               ( [ "a" ],
                 [ Unify (Skolem "a", Int) ],
                 Function
                   (Skolem "a", Application (Constructor "HasUnify", Skolem "a"))
               ) ));
   let t, c =
     Infer.infer environment
       (Case
          ( Apply (Constructor "HasUnify", Int 21),
            PtApply (PtConstructor "HasUnify", [ PtVariable "a" ]),
            Variable "a" ))
   in
   let c = Solver.solve environment c in
   print_endline @@ Pretty.render_ty @@ Type.normalize t;
